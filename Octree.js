//MODIFIED VERSION OF https://vorg.github.io/pex/docs/pex-geom/Octree.html
//https://github.com/pantsbuild/pex/blob/master/LICENSE
...
Octree.prototype.findNearestPoint = function (p, options = {includeData : true, maxDist : Infinity, notSelf : false}) {
  ...
  if (this.children.length > 0) {
    ...
    for (var i = 0; i < this.points.length; i++) {
      ...
      //var distSq = p.squareDistance(o);
		var distSq = p.subtract(o);
		distSq.x *= distSq.x;
		distSq.y *= distSq.y;
		distSq.z *= distSq.z;
		distSq = distSq.x + distSq.y + distSq.z;
...
Octree.Cell.prototype.split = function () {
  ...
  this.children.push(new Octree.Cell(this.tree, new Vec3(x, y, z), new Vec3(w2, h2, d2), this.level + 1));
  this.children.push(new Octree.Cell(this.tree, new Vec3(x + w2, y, z), new Vec3(w2, h2, d2), this.level + 1));
  this.children.push(new Octree.Cell(this.tree, new Vec3(x, y, z + d2), new Vec3(w2, h2, d2), this.level + 1));
  this.children.push(new Octree.Cell(this.tree, new Vec3(x + w2, y, z + d2), new Vec3(w2, h2, d2), this.level + 1));
  this.children.push(new Octree.Cell(this.tree, new Vec3(x, y + h2, z), new Vec3(w2, h2, d2), this.level + 1));
  this.children.push(new Octree.Cell(this.tree, new Vec3(x + w2, y + h2, z), new Vec3(w2, h2, d2), this.level + 1));
  this.children.push(new Octree.Cell(this.tree, new Vec3(x, y + h2, z + d2), new Vec3(w2, h2, d2), this.level + 1));
  this.children.push(new Octree.Cell(this.tree, new Vec3(x + w2, y + h2, z + d2), new Vec3(w2, h2, d2), this.level + 1));
  for (var i = 0; i < this.points.length; i++) {
...
Octree.Cell.prototype.findNearestPoint = function (p, options) {
  ...
  if (this.points.length > 0 && this.children.length == 0) {
    for (var i = 0; i < this.points.length; i++) {
	  ...
	  var dist = this.points[i].subtract(p);
	  dist.x *= dist.x;
	  dist.y *= dist.y;
	  dist.z *= dist.z;
	  dist = Math.sqrt(dist.x + dist.y + dist.z);
      if (dist <= bestDist) {
  ...
  if (children.length > 0) {
    for (var i = 0; i < children.length; i++) {
      ...
      if (child.points.length > 0) {
		...
		var childNearestDist = childNearest.point.subtract(p);
	  	childNearestDist.x *= childNearestDist.x;
	  	childNearestDist.y *= childNearestDist.y;
	  	childNearestDist.z *= childNearestDist.z;
	  	childNearestDist = Math.sqrt(childNearestDist.x + childNearestDist.y + childNearestDist.z);
